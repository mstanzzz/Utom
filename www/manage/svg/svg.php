...
if(isset($_POST['add_svg'])){
	...
	$sql = "INSERT INTO svg 
			(name, svg_code, profile_account_id) 
			VALUES 
			('".$name."','".$svg_code."','".$_SESSION['profile_account_id']."')";
...
<a onClick="regularSubmit();" href="#" class="btn btn-success btn-large"><i class="icon-ok icon-white"></i> Save Changes </a>
	...
	while($img_row = $result->fetch_object()){
		...
		$block .= "<td valign='middle' class='center'>
			...
			<input type='radio' class='radioinput' name='logo_id' value='".$img_row->svg_id."' $checked /></div></td>";		
