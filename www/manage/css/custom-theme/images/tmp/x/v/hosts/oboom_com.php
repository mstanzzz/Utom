...
class dl_oboom_com extends Download {
    ...
    public function CheckAcc($cookie){
        ...
        if (preg_match('@premium_unix"\:([^,]+)@i', $data, $redir)) {
                        ...
                        elseif (preg_match('@traffic"\:{"current"\:([^,]+),"increase"\:[^,]+,"last"\:[^,]+,"max"\:([^}]+)@i', $data, $redir2)) {
                                return array(true, 'Until ' .date('H:i:s Y-m-d', $redir[1]). '<br/> Traffic available: ' .round($redir2[1]/1073741824, 2). ' GB<br/> Max: ' .round($redir2[2]/1073741824, 2). ' GB');
                        }
                }
    ...
    public function login($dbCustom,$user, $pass){
                ...
                $page = $this->lib->curl('https://www.oboom.com/1/login', 'lang=EN', $post, 0);
    ...
    public function Leech($link) {
                ...
                if (preg_match('@https?://(www.)?oboom\.com/([\w]{8})@i', $link, $id)) $link = "https://www.oboom.com/$id[2]";
                ...
                if (!preg_match('@https?://[\w]+\.oboom\.com/1\.0/dlh\?ticket=[^\r\n]+@i', $page, $dlink)) {
                        if (!preg_match('@Redirecting to /#([\w]{8})@i', $page, $redir3)) $this->error('Item ID not found.', true, false);
                       
                        $page = $this->lib->curl('https://www.oboom.com/#' .trim($redir3[1]), $this->lib->cookie, '');
                       
                        if (!preg_match('@Session : "([^"]+)"@i', $page, $token)) $this->error('Token not found.', true, false);
                       
                        $page = $this->lib->curl('https://api.oboom.com/1/dl', $this->lib->cookie, array('token' => $token[1], 'item' => $redir3[1],), 0);
                        ...
                        elseif (isset($json[0]) && $json[0] != 200) {
                                $code = $json[0];
                               
                                if (is_numeric($code)) {        //Th3-822
                                        switch ($code) {
