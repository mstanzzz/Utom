...
class dl_depositfiles_com extends Download { 
	...
	public function CheckAcc($cookie){
		SITEROOT = $this->lib->cut_str($this->getredirect("http://depositfiles.com/"), "//", "/");
		$data = $this->lib->curl("http://{SITEROOT}/gold/payment_history.php", "lang_current=en;{$cookie}", "");
		if(stristr($data, 'You have Gold access until:'))  {
			$checksubscribe = $this->lib->curl("http://{SITEROOT}/gold/payment_subscribe_manage.php", "lang_current=en;{$cookie}", "");
    ...
    public function Leech($url) {
		...
		list($name, SITEROOT) = explode(".", $this->lib->cut_str(str_replace("www.", "", $url), "http://", "/"));
		...
		if($pass) {
			...
			elseif(preg_match('@https?:\/\/fileshare\d+\.'.$name.'\.'.SITEROOT.'\/auth\-[^"\'<>\r\n\t]+@i', $data, $link))
			return trim($link[0]);
		}
    ...
    public function Leech($url) {
		...
		elseif($page['status'] == "OK" && isset($page['data']['download_url'])) return trim(str_replace("depositfiles.com", SITEROOT, $page['data']['download_url']));
