...
foreach ($top_cats as $top_cat) {
	...
	$block .= "<a tabindex='-1' class='tree-parent tree-parent-collapsed' onclick='show_children(".$top_cat['cat_id'].")'  data-catid='".$top_cat['cat_id']."' data-cattype='topcat'><img src='".$ste_root."/saascustuploads/".$_SESSION['profile_account_id']."/cart/thumb/".$top_cat['file_name']."' />".stripAllSlashes($top_cat['name']).'';
	...
	$block .= "<ul role='group' class='childrenplaceholder'>".getChildren($top_cat['cat_id'], $domain, $item_id, $max_depth)."</ul></li>"; 
}
echo $block;
function getChildren($cat_id, $domain, $item_id, $max_depth){
	$max_depth--;
	...
	while($row = $result->fetch_object()) {
		...
		$block .= "<ul role='group' class='childrenplaceholder'>".getChildren($row->cat_id, $domain, $item_id, $max_depth)."</ul></li>";
	}	
	return $block;
}
?>