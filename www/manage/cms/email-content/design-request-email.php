...
if($result->num_rows == 0){
	$sql = "INSERT INTO design_email_content 
		(profile_account_id, content) 
		VALUES ('".$_SESSION['profile_account_id']."', 'none')"; 
	$result = $dbCustom->getResult($db,$sql);
...
if($result->num_rows > 0){
	...
	$design_email_content_id = $object->design_email_content_id;
	//$content = $object->content;
}else{
	$to_cust_email_body = '';
	$design_email_content_id = 0;
	//$content = '';
}
...
<div class="manage_page_container">
	...
	<form name="form" action="email-content-landing.php" method="post" enctype="multipart/form-data">
        ...
        <input type="hidden" name="design_email_content_id" value="<?php echo $design_email_content_id; ?>">     
