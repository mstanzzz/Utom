...
// add if not exist
$sql = "SELECT diy_instructions_id FROM diy_instructions WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
...
if($result->num_rows == 0){
	...
	$result = $dbCustom->getResult($db,$sql);	
	$_SESSION['diy_instructions_id'] = $db->insert_id;
}else{
	$_SESSION['diy_instructions_id'] = (isset($_REQUEST['diy_instructions_id'])) ? $_REQUEST['diy_instructions_id'] : 0;
}
if(!is_numeric($_SESSION['diy_instructions_id'])) $_SESSION['diy_instructions_id'] = 0;

$msg = (isset($_GET['msg'])) ? $_GET['msg'] : '';

if(isset($_POST['update_diy_instructions'])){

	$_SESSION['diy_instructions_id'] = isset($_POST['diy_instructions_id'])? $_POST['diy_instructions_id'] : 0;
	if(!is_numeric($_SESSION['diy_instructions_id'])) $_SESSION['diy_instructions_id'] = 0;
	...
	$stmt = $db->prepare("UPDATE diy_instructions
						SET
						top_1 = ?
						,top_2 = ?
						,top_3 = ?						
...
WHERE diy_instructions_id = '".$_SESSION['diy_instructions_id']."'";
$result = $dbCustom->getResult($db,$sql);	
...
<div class="manage_page_container">
	...
	<form name="form" action="<?php echo $current_page; ?>" method="post" enctype="multipart/form-data">
		<input type="hidden" name="update_diy_instructions" value="1">        
		<input type="hidden" name="diy_instructions_id" value="<?php echo $_SESSION['diy_instructions_id']; ?>">
