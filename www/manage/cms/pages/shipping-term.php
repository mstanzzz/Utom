...
$page_title = "Edit Shipping Term";
$page_group = "shipping-term";
$page = "shipping";
...
// add if not exist
$sql = "SELECT shipping_term_id FROM shipping_term WHERE profile_account_id = '".$_SESSION['profile_account_id']."'"; 
...
if(isset($_POST["edit_shipping_term"])){
	
	$content = trim(addslashes($_POST["content"])); 
	$shipping_term_id = $_POST["shipping_term_id"];
	...
	//if(in_array(2,$user_functions_array)){
		// create a backup
		//$backup = new Backup;
		//$dbu = $backup->doBackup($shipping_term_id,$user_id,"shipping_term");	
		$sql = sprintf("UPDATE shipping_term SET content = '%s', img_id = '%u', img_alt_text = '%s' WHERE shipping_term_id = '%u'", 
		$content, $img_id, $img_alt_text, $shipping_term_id);
	...
	/*
		...
		$sql = sprintf("INSERT INTO review (content_table, when_submitted, submitted_by_login_id, slug, content1, img_id, content_record_id) 
			...
			"shipping_term", $ts, $user_id, "shipping-term", $content, $img_id, $shipping_term_id);
...
$result = $dbCustom->getResult($db,$sql);	
	...
	$this_shipping_term_id = (isset($_GET['shipping_term_id'])) ? $_GET['shipping_term_id'] : 0;
	if(!isset($_SESSION['shipping_term_id'])) $_SESSION['shipping_term_id'] = $this_shipping_term_id; 

 	$db = $dbCustom->getDbConnect(SITE_N_DATABASE);
	$sql = "SELECT content, img_id, heading, img_alt_text 
    FROM shipping_term
 	WHERE shipping_term_id = '".$_SESSION["shipping_term_id"]."'";
    $result = $dbCustom->getResult($db,$sql);	
	if($result->num_rows > 0){
		$object = $result->fetch_object();
		$intro = '';
...
function ajax_set_page_session(){
		...
		var q_str = "?page=shipping-term"+get_query_str();
		//alert(q_str);
...
function validate(theform){
	if(theform.name == "add_shipping_term"){
		//alert(theform.name);
...
<div class="manage_page_container">
	...
	<div class="manage_main">
		...
		$bread_crumb->add("Shipping", '');
        echo $bread_crumb->output();

        require_once($_SERVER['DOCUMENT_ROOT'].'/manage/admin-includes/manage-content-top.php');
        ?>
		<form id="form" name="form" action="<?php echo $current_page; ?>" method="post" enctype="multipart/form-data">
            <input type="hidden" name="edit_shipping_term" value="1">        
            <input type="hidden" name="shipping_term_id" value="<?php echo $_SESSION["shipping_term_id"]; ?>">
            <input type="hidden" name="ret_page" value="shipping-term">        
            <input type="hidden" name="ret_dir" value="manage/cms/pages">
			<input type="hidden" name="page" value="<?php echo $page; ?>"> 
            <input type="hidden" name="content_table" value="shipping_term">          
				...
				<fieldset class="edit_images">
					...
					?>
                        ...
                        ?>
                            ...
                            href="<?php echo $ste_root;?>/manage/cms/upload.php?ret_page=shipping-term&ret_dir=pages&img_max_width=450">
 ...
 <p class="clear"></p>
  ...
  <div id="content-delete" class="confirm-content">
	...
	<form name="del_img_form" action="shipping-term.php" method="post" target="_top">
