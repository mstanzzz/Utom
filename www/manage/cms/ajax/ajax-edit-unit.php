...
$i=0;
while($row = $result->fetch_object()) {	
	$qty_schema_array[$i][''] = $row->qty_schema_id;
    	...
    	<div class="clearfix">
        	<div class="left-side">
				<div class="clearfix edit-unit-detail-label">
                    ...
                    <input type="text" class="input-border-bottom input-italic" id="edit_unit_name" name="unit_name" value="<?php echo $_SESSION['tmp_vars']['unit_name'];  ?>"/>
				</div>
				<div class="clearfix edit-unit-detail-label">
                    <label class="modal-tool-admin-label">Part Type</label>
                    <select id="edit_part_type_id" name="part_type_id" 
                ...
                <div class="clearfix edit-unit-detail-label" style="padding-top:20px; padding-bottom:20px;">
                    <button type="button" 
                    onClick="open_modal_add_unit_component_edit();" 
                    class="btn-default btn-small with-bottom-shadow btn-with-border btn-bold btn-italic btn-modify">
                    Add Component
                    </button>
                    <button type="button"                                  
                    class="btn-default btn-small with-bottom-shadow btn-with-border btn-bold btn-italic btn-modify"
                    onClick="open_modal_by_id('modify_component');">                                
                    Modify
                    </button>
                    <table id="component_list_unit_edit" width="100%" cellpadding="10">
                    <?php
                    if(count($_SESSION['component_array']) > 0){
                    echo "<tr><td width='80%'>Name</td><td>QTY</td><td></td></tr>";	
                    foreach($_SESSION['component_array'] as $v){
                    echo '<tr><td>'.stripslashes($v['component_name']).'</td><td>'.$v['qty'].'</td>';
                    echo "<td><span style='cursor:pointer; color:red;' onClick='delete_component_from_unit(1,".$v['component_id'].")'>delete</span></td>";		
                    }
                    }
                    ?>
                    </table>
                ...
                <div class="label-with-dropdown clearfix">
                    ...
                    id="edit_collection_ids" name='collection_ids[]' 
                    class="btn-default btn-small text-capitalize with-small-bottom-shadow with-small-dropdown">             
                    <option value="0">Select</option>    
                    <?php
                ...
                <div class="separator-text-input">
                            ...
                            <input type="text" class="input-border-bottom" id="edit_unit_image" name="unit_image" value="<?php echo $_SESSION['tmp_vars']['unit_image']; ?>" />
                </div>
                <div class="separator-text-input">
                            <label class="modal-tool-admin-label unit-label">Unit SKU</label>
                            <input type="text" class="input-border-bottom" id="edit_unit_sku" name="unit_sku" value="<?php echo $_SESSION['tmp_vars']['unit_sku']; ?>"/>
                </div>
                <div class="separator-text-input">
                            <label class="modal-tool-admin-label unit-label">Unit Number</label>
                            <input type="text" class="input-border-bottom" id="edit_unit_number" name="unit_number" value="<?php echo $_SESSION['tmp_vars']['unit_number']; ?> "/>
                </div>
                <div class="separator-text-input">
                            <label class="modal-tool-admin-label unit-label">Unit Weight</label>
                            <input type="text" class="input-border-bottom" id="edit_unit_weight" name="unit_weight" value="<?php echo $_SESSION['tmp_vars']['unit_weight']; ?> "/>
                </div>
                <div class="separator-text-input">
                            <label class="modal-tool-admin-label unit-label">Unit Price</label>
                            <input type="text" class="input-border-bottom" id="edit_price_unit" name="price_unit" value="<?php echo $_SESSION['tmp_vars']['price_unit']; ?> "/>
                </div>
                <div class="separator-text-input">
                            <label class="modal-tool-admin-label unit-label">Unit QTY</label>
                            <input type="text" class="input-border-bottom" id="edit_qty_unit" name="qty_unit" value="<?php echo $_SESSION['tmp_vars']['qty_unit']; ?> "/>
                </div>
                <div class="separator-text-input">
                            <label class="modal-tool-admin-label unit-label">Description</label>
                            <input type="text" class="input-border-bottom" id="edit_descriptions" name="description" value="<?php echo $_SESSION['tmp_vars']['description']; ?>"/>
				</div>
			</div>
...
<div id="modal_add_unit_component_edit" class="edit-container add-new-scheme-parameter"
	style="visibility:hidden; position:absolute; top:200px; left:64%; z-index:22;">
	<form>
	<div class="edit-form-wrapper no-border">
        <label class="text-italic">Add new Component</label>
        <select id="unit_component_id_edit" class="rounded-select" style="width:180px;">
            <option value="0">Select Options</option>
            <?php
            foreach($component_array as $val){
    ...
    <div class="edit-form-wrapper sort-order no-border">
        ...
        <input type="text" id="unit_component_qty_edit" class="greyed-input input-width-140">
    </div>
    <div class="btn-wrapper-center">
        <button type="button" class="btn-default btn-bold btn-mint-border with-bottom-shadow"
        onClick="close_modal_add_unit_component_edit();">
	        Cancel
        </button>
        <button type="button" class="btn-default btn-bold btn-mint-border with-bottom-shadow"
        onClick="add_component_to_unit(1);">
    	    Add
        </button>
    </div>
    </form>
