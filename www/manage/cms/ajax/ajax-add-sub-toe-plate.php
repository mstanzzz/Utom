...
$db = $dbCustom->getDbConnect(COMPONENTS_DATABASE);
	...
	$stmt = $db->prepare("INSERT INTO cabinetry_toe_plates
						(toe_plate_name
						,collection_id
						,qty_schema_id
						,qty_calc_id
		...
		if(!$stmt->bind_param('siiiiiiiii'
				...
				$toe_plate_id = $db->insert_id;
				
				$sql = "SELECT table_name_id 
						FROM table_names
						WHERE table_name = 'cabinetry_toe_plates'";
				...
				if($result->num_rows > 0){
					...
					$sql = "INSERT INTO default_design_elements
						...
						VALUES
						('".$object->table_name_id."', '".$toe_plate_id."', '".$_SESSION['profile_account_id']."', '1')";
