...
$_SESSION['ret_modal'] = '';
	...
	$component_image = (isset($_GET['component_image']))? trim(addslashes($_GET['component_image'])) : '';
...
$db = $dbCustom->getDbConnect(COMPONENTS_DATABASE);
	...
	$collection_ids = (isset($_POST['collection_ids']))? $_POST['collection_ids'] : array();


	$stmt = $db->prepare("INSERT INTO cabinetry_components
						(component_name
                        ,component_image
                        ,component_sku
                        ,component_number
	...
	if(!$stmt->bind_param('sssssddiiiiiiiiiiiii'
				...
				$sql = "SELECT table_name_id 
						FROM table_names
						WHERE table_name = 'cabinetry_components'";
				...
				if($result->num_rows > 0){
					...
					$sql = "INSERT INTO default_design_elements
						...
						VALUES
						('".$object->table_name_id."', '".$component_id."', '".$_SESSION['profile_account_id']."', '1')";
