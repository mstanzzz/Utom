...
if($first_load > 0){
	...
	$sql = "SELECT cabinetry_components_to_parts.part_id
			...
			AND cabinetry_components_to_parts.component_id = '".$component_id."'";
    $result = $dbCustom->getResult($db,$sql);
	$i = 0;
	while($row = $result->fetch_object()){
...
if($_SESSION['tool_image'] != ""){	
	$_SESSION['tmp_vars']['component_image'] = $_SESSION['tool_image'];
}
...
<input type="hidden" id="update_this_component_id" name="component_id" value="<?php echo $component_id; ?>" />
	    ...
	    <div class="left-side">
			...
			<div class="plate-detail clearfix cleat-name-label">
				...
				<button  type="button" class="btn-default btn-small with-small-bottom-shadow btn-with-border btn-plate-detail add-constructed-parts" 
					onClick="open_add_sub_cmp_constructed_part_edit();">
						Add Constructed Part
				</button>
			</div>

			<ul id="ajx_sub_cmp_part_list_edit">
            <?php
            ...
            <div class="plate-detail clearfix cleat-name-label">
                ...
                <button type="button" 
	                ...
	                onclick="open_add_sub_cmp_fixed_part_edit();">
                		Add Catalog Part
                </button>
            </div>

			<ul id="ajx_sub_cmp_fixed_part_list_edit">
				<?php
