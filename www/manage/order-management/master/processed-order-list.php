	...
	<form id='orders_by_profile_form' name='orders_by_profile_form' action='order-list.php' method='post'>
			<div class="page_actions">
                ...
                if($total_rows > $rows_per_page){
						echo getPagination($total_rows, $rows_per_page, $pagenum, $truncate, $last, "order-management/master/processed-order-list.php", $sortby, $a_d);
					echo "<br /><br /><br />";
				}
				?>
					...
					<?php
						...
						while($row = $result->fetch_object()) {
							...
							*/
							...
							$sql = "SELECT name
									FROM order_state
									WHERE order_state_code = '".$row->order_state_code."'";
							...
							$block .= "</td>";
							//View
							$block .= "<td><a href='order.php?order_id=".$row->order_id."&ret=processed-order-list' class='btn btn-small'><i class='icon-eye-open'></i> View</a>"; 
							//process
							$block .= "<td><a href='order-list.php?order_id=".$row->order_id."&unprocess=1' class='btn btn-primary btn-small'>
							<i class='icon-check icon-white'></i>Un-Process</a></td></tr>"; 
						}
							echo $block;
						?>
				</table>
