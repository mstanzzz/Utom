...
class ShoppingCartItem {
	...
	function getItem($dbCustom,$item_id, $profile_item_id = 0) {
		...
		if($result->num_rows > 0){
			...
			$ret['brand_name'] = $this->getBrandNameFromBrandId($dbCustom,$object->brand_id);
			$ret['file_name'] = $this->getFileNameFromImgId($dbCustom,$object->img_id);
	...
	function getBrandNameFromBrandId($dbCustom,$brand_id){
	...
	function getFileNameFromImgId($dbCustom,$img_id){
		$db = $dbCustom->getDbConnect(CART_DATABASE);
	...
	function getFileNameFromItemId($dbCustom,$item_id){
	...
	function getStyleNameFromId($dbCustom,$style_id){
