...
function signIn2(){
	...
	$.ajax({
	  url: '<?php echo $ajax_$_SERVER['DOCUMENT_ROOT']; ?>/ajax-sign-in.php?user='+user+'&password='+password,
...
function send_password_reset(){
	...
	if(isValidEmail(email_addr)){
		$.ajaxSetup({ cache: false}); 
		$.ajax({
		  url: '<?php echo $ajax_$_SERVER['DOCUMENT_ROOT']; ?>/ajax-send-reset-pasword.php?email_addr='+email_addr,
		  success: function(data) {
			if(data.indexOf("y") > -1){
	...
	<section class="row gutter-top">
		<div class="span12">
				...
				<div id="signin_replace_2" class="sign-in-form">
						...
						<button class="btn btn-primary" id="register_button_2" onClick="window.location.href='<?php echo $_SERVER['DOCUMENT_ROOT'];?>/signup-form.html'">Register</button>
				</div>
