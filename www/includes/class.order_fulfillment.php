...
class OrderFulfillment
{
	...
	public function initializeAllLiSteps($order_line_item_id){
		...
		if(!is_numeric($order_line_item_id)) $order_line_item_id = 0;
		
		//echo "<br />";
		//echo "in initializeAllLiSteps";
		//echo "<br />";
		
		$dbCustom = new DbCustom();				
		$db = $dbCustom->getDbConnect(CART_DATABASE);
		
		$sql = "DELETE FROM fulfill_step_to_line_item
				WHERE order_line_item_id = '".$order_line_item_id."'";
		$result = $dbCustom->getResult($db,$sql);
					
		$all_steps = $this->getAllSteps();
				
		foreach($all_steps as $val){
	...
	public function getMinIdByNumber($step_number){
		...
		if(!is_numeric($step_number)) $step_number = 0;
	...
	public function getDesignFulfillmentStepData($order_fulfillment_step_id, $order_line_item_id){
		...
		if(!is_numeric($order_fulfillment_step_id)) $order_fulfillment_step_id = 0;
		if(!is_numeric($order_line_item_id)) $order_line_item_id = 0;
	...
	public function hasStepRecord($order_line_item_id){
	...
	public function parkOrder($order_id){
		...
		foreach($line_items_array as $val){
			...
			$tmp = $this->getMaxStartedStepIDPerLineItem($val['order_line_item_id']);
						
			$this->parkStep($val['order_line_item_id'], $tmp);
	...
	public function parkStep($order_line_item_id, $order_fulfillment_step_id, $parked_by_user_id = 0){
				
		if(!is_numeric($order_line_item_id)) $order_line_item_id = 0;
		if(!is_numeric($order_fulfillment_step_id)) $order_fulfillment_step_id = 0;
		if(!is_numeric($parked_by_user_id)) $parked_by_user_id = 0;
		
		$dbCustom = new DbCustom();				
		$db = $dbCustom->getDbConnect(CART_DATABASE);

		$sql = "UPDATE fulfill_step_to_line_item
				SET is_parked = '1', parked_by_user_id = '".$parked_by_user_id."'
				WHERE order_line_item_id = '".$order_line_item_id."'
				AND order_fulfillment_step_id = '".$order_fulfillment_step_id."'";					
		$result = $dbCustom->getResult($db,$sql);		
	...
	public function setStepStart($order_line_item_id, $order_fulfillment_step_id, $started_by_user_id = 0){
		if(!is_numeric($order_line_item_id)) $order_line_item_id = 0;
		if(!is_numeric($order_fulfillment_step_id)) $order_fulfillment_step_id = 0;
		if(!is_numeric($started_by_user_id)) $started_by_user_id = 0;
		...
		$result = $dbCustom->getResult($db,$sql);
		if($result->num_rows == 0){
			$this->initializeAllLiSteps($order_line_item_id);
		}
	
		$sql = "UPDATE fulfill_step_to_line_item
				SET when_started = '".$ts."', started_by_user_id = '".$started_by_user_id."'
				WHERE order_line_item_id = '".$order_line_item_id."'
				AND order_fulfillment_step_id = '".$order_fulfillment_step_id."'";				
	...
	public function setFinished($order_line_item_id, $order_fulfillment_step_id, $completed_by_user_id = 0){

		if(!is_numeric($order_line_item_id)) $order_line_item_id = 0;
		if(!is_numeric($order_fulfillment_step_id)) $order_fulfillment_step_id = 0;
		if(!is_numeric($completed_by_user_id)) $completed_by_user_id = 0;
		...
		if($this->isStarted($order_line_item_id, $order_fulfillment_step_id)){
			...
			$sql = "UPDATE fulfill_step_to_line_item
					SET when_finished = '".$ts."', completed_by_user_id = '".$completed_by_user_id."'
					WHERE order_line_item_id = '".$order_line_item_id."'
					AND order_fulfillment_step_id = '".$order_fulfillment_step_id."'";					
			$result = $dbCustom->getResult($db,$sql);
		}
	...
	public function getMaxStartedStepNamePerLineItem($order_line_item_id){
		...
		$max_started_step_id = $this->getMaxStartedStepIDPerLineItem($order_line_item_id);

		$sql = "SELECT step_name ,step_number 
				FROM order_fulfillment_step
				WHERE order_fulfillment_step_id = '".$max_started_step_id."'";
		$result = $dbCustom->getResult($db,$sql);
		
		if($result->num_rows > 0){
			$object = $result->fetch_object();			
			return 'Step:'.$object->step_number.' '.$object->step_name;
		}
	...
	public function getMaxStartedStepIDPerLineItem($order_line_item_id){
		...
		$sql = "SELECT MAX(order_fulfillment_step_id) AS max_step_id
				FROM fulfill_step_to_line_item
				WHERE order_line_item_id = '".$order_line_item_id."'
				AND when_started > '0'";		
		$result = $dbCustom->getResult($db,$sql);
	...
	public function getMaxFinishedStepIDPerLineItem($order_line_item_id){
	...
	public function getMaxStartedStepNumberPerOrder($order_id){
	...
	public function getMaxFinishedStepNumberPerOrder($order_id){
		...
		foreach($line_items_array as $val){
			...
			$tmp = $this->getMaxFinishedStepIDPerLineItem($val['order_line_item_id']);		
	...
	public function getMaxStartedStepName($order_id){

		if(!is_numeric($order_id)) $order_id = 0;
	
		$max_step = $this->getMaxStartedStepNumberPerOrder($order_id);
	...
	function is_line_item_fulfilled($order_line_item_id){
		...
		while($row = $result->fetch_object()){
			...
			//echo "isFinished:  ".$this->isFinished($order_line_item_id, $row->order_fulfillment_step_id);
			//echo "<br />";

			if($this->isFinished($order_line_item_id, $row->order_fulfillment_step_id) == 0){
				return 0;
			}
		}		
	...
	function is_order_fulfilled($order_id){
		...
		while($row = $result->fetch_object()){			
			...
			//echo "is_line_item_fulfilled:  ".$this->is_line_item_fulfilled($row->order_line_item_id);
			//echo "<br />";
			
			if($this->is_line_item_fulfilled($row->order_line_item_id) == 0){
				return 0;				
			}	
		}
	...
	public function getAllSteps($is_desc = 0){
	...
	public function isEmpAssignedToStep($order_line_item_id, $order_fulfillment_step_id, $employee_id=0){
	...
	public function addStepEmployee($order_line_item_id, $order_fulfillment_step_id, $employee_id=0){
	...
	public function removeStepEmployee($order_line_item_id, $order_fulfillment_step_id, $employee_id=0){
	...
	public function getAllEmpPerStep($order_line_item_id, $order_fulfillment_step_id){
	...
	public function getRestriction($condition, $subject_step_id, $condition_step_id){
		...
		if(!is_numeric($subject_step_id)) $subject_step_id = 0;
		if(!is_numeric($condition_step_id)) $condition_step_id = 0;
	...
	public function getStepIdFromStepNum($step_number){
		...
		if($result->num_rows > 0){
			$object = $result->fetch_object();
			return $object->order_fulfillment_step_id;
		}
