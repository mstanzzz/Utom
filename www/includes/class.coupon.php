...
class Coupon {
	...
	function getCoupon($dbCustom,$code,$amount){
	...
	function coupon_exists($dbCustom,$code){	
		$db = $dbCustom->getDbConnect(CART_DATABASE);
		$ret = 0;
	...
	function getCouponArray($dbCustom,$code,$amount){
	...
	function hasBeenUsed($dbCustom,$code,$customer_id){		
		$db = $dbCustom->getDbConnect(CART_DATABASE);
		$ts = time();
