...
if(isset($_POST['update_folder_id'])){
	
	$idea_folder_id = (isset($_POST['update_folder_id'])) ? $_POST['update_folder_id'] : 0;
	if(!is_numeric($idea_folder_id)) $idea_folder_id = 0;
	
	$name = (isset($_POST['folder_name'])) ? $_POST['folder_name'] : '';

	$stmt = $db->prepare("UPDATE idea_folder
						SET name = ?
						WHERE idea_folder_id = ?"); 
		
		echo "<br />";
		echo 'Error INSERT   '.$db->error;
		echo "<br />";
		
	if(!$stmt->bind_param("si", $name,$idea_folder_id)){
...
if(isset($_POST['del_folder_id'])){
	
	$idea_folder_id = (isset($_POST['del_folder_id'])) ? $_POST['del_folder_id'] : 0;	
	if(!is_numeric($idea_folder_id)) $idea_folder_id = 0;
...
if(isset($_POST['add_edit_new_folder'])){
	
	$name = (isset($_POST['folder_name'])) ? $_POST['folder_name'] : '';

	$stmt = $db->prepare("INSERT INTO idea_folder
		(profile_account_id
		,user_id
		,name)
		VALUES
