...
$idea_folder_id = (isset($_GET['idea_folder_id'])) ? $_GET['idea_folder_id'] : 0;

$db = $dbCustom->getDbConnect(CART_DATABASE);

if(isset($_POST['del_house_id'])){
...
$sql = "SELECT idea_house.house_name
		...
		FROM idea_house
		WHERE idea_house.idea_folder_id = '".$idea_folder_id."'
	";
...
while($row = $result->fetch_object()){
	$houses_array[$i]['idea_house_id'] = $row->house_name;
...
$rooms_block .= "<input type='checkbox' class='check custom-checkbox' "; 
$rooms_block .= " id='checkbox-".$row->idea_room_id."' value='".$row->idea_room_id."'>";
$rooms_block .= "<label for='checkbox-1'>Living Room</label>";
$rooms_block .= "</div>";
